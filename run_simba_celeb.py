...
model.eval()
image_size = 299 # for inception_v3
testset = dset.ImageFolder(args.data_root + "/celeba", utils.INCEPTION_TRANSFORM)
# testset = dset.CelebA(root=args.data_root, split="all", download=True, transform=utils.INCEPTION_TRANSFORM)
attacker = SimBA(model, 'celeba', image_size)
...
if os.path.isfile(batchfile):
    ...
    while preds.ne(labels).sum() > 0:
        ...
        preds[idx], _ = utils.get_preds(model, images[idx], 'celeba', batch_size=args.batch_size)
